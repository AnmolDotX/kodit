...
function Editor() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      function handleErrors(e) {
        ...
        toast.error("Connection to server failed, try again later.", e.message);
        navigate("/");
      }
  ...
  const RunCode = async () => {
    try {
      ...
      toast.error("Error running code : ", err);
    ...
    <>
      ...
      >
        <div className='w-full h-full flex bg-slate-950/90 backdrop-filter backdrop-blur-sm'>
          ...
          <aside
            className={`bg-emerald-950/20 backdrop-filter backdrop-blur-lg md:w-[20vw] h-full md:flex flex-col justify-between py-4 px-4 ${
