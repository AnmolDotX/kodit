        ...
        <>
            ...
            <div className='w-full h-[100vh]' style={{
                ...
                <aside className='text-white pl-2 pt-2 md:hidden'>
                    <button className='font-bold text-2xl text-orange-700 rounded-md px-2 pb-1 transition-all' onClick={() => setMenu(!menu)}>{menu ? "X" : "="}</button>
                </aside>

                <aside className={`bg-emerald-950/20 backdrop-filter backdrop-blur-lg md:w-[20vw] h-full md:flex flex-col justify-between py-4 px-4 ${menu ? phoneStyle : 'hidden'}`}>
                    <div>
                        <div className='flex'>
                            <AppTitle />
                        ...
                        <div className='text-white mt-2'>
                            <h2 className='font-bold my-4 text-orange-400'>connected users</h2>
                            {/* connected clients */}
                    ...
                    <div className='flex flex-col gap-2 mt-2'>
                        <button className='bg-black/50 border-emerald-500/50 border text-sm text-white hover:text-sky-600 px-10 py-2 rounded-md transition-all' onClick={copyRoomId}>Copy Room Id</button>
                        <button className='bg-gradient-to-br from-red-500/30 border border-red-500/50 to-transparent backdrop-filter backdrop-blur-sm shadow hover:shadow-lg hover:text-red-200 px-10 py-2 rounded-md text-white font-bold transition-all duration-300' onClick={leave}>Leave</button>
                    </div>
                ...
                <aside className='flex w-[80vw] flex-col'>
                    <div className='flex justify-center'>
                        <button className='bg-green-600/50 backdrop-filter backdrop-blur-sm hover:shadow-green-700 shadow hover:shadow-2xl transition-all hover:bg-emerald-600/40 px-10 py-2 rounded-md my-2 text-white font-bold active:bg-emerald-600/80 active:shadow-green-600' onClick={RunCode}>{'> Run'}</button>
                    </div>
                    <div className='w-full rounded-md'>
                        <EditorComponent socketRef={socketRef} roomId={roomId} onCodeChange={(code) => codeRef.current = code} />
                    </div>
                    <div className='h-[40vh]'>
                        <textarea className='w-full h-[95%] bg-black/60 backdrop-filter backdrop-blur-sm text-white outline-none rounded-md px-6 my-2' placeholder='output:' id="OutputBox" ref={outPutRef}>
                        </textarea>
                    </div>
